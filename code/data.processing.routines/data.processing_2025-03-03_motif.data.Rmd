...
output_dir <- "output"
data_new <- read.csv(file.path(git_root, "data", "song", "motifs", "motif.data_2025-03-03.csv")) %>% tibble()
```
...
motif_data_detailed <- motif_data_renamed %>% filter(!(is.na(file) & is.na(song_start)))
motif_data_misc <- motif_data_renamed %>%
    mutate(male = str_replace(male, "_misc_motifs", "") ) #filter((is.na(file) & is.na(song_start)))
...
motif_data <- motif_data_detailed |>
    ...
    ungroup() |>
    select(!c(start_song, end_song, date_file)) |>
...
write_csv(motif_wrong_file, file = "need.checking_2025-03-10.csv")
...
motif_summary <- motif_data |>
  ...
  summarize(song_count = max(song_index),
            ...
            motif_length_mean = song_length_sum/motif_sum,
            motif_length_sd = sd(song_length/motifs),
...
save_obj <- c("motif_data", "motif_data_detailed", "motif_data_misc", "motif_summary", "nonsong_motif_data")
