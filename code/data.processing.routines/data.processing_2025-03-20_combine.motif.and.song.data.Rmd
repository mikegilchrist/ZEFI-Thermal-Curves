...
```{r}
load(file.path("output", "data.processing_2025-03-20_process.song.data.Rda"), verbose = TRUE)
...
chamber_info <- motif_data_full[ , c("male", "date", "trial_round", "temp_target", "temp_mean", "humidity_mean", "vpd", "chamber", "counter", "test_order", "mass")]

## Add trial information to song data
song_motif_chamber_data <- left_join(song_data, chamber_info, by = c("male", "date"))
...
write_csv(x=song_motif_chamber_data, file = file.path("output", "song_motif_chamber_data_2025-03-20.csv"))
```
...
not_equal_motif <- which(data$motif_count.x!=data$motif_count_song_only) ## does not include misc_motifs
too_few_motifs <- which(data$motif_count_song_only < data$motif_count.y)

data[not_equal_count, c("male", "date", "motif_count.x", "motif_count.y", "motif_count_song_only", "song_count")] |> print(n=150)

data[not_equal_motif, c("male", "date", "motif_count.x", "motif_count.y", "motif_count_song_only", "song_count")] |> group_by(male, date) |>
    arrange(male, .by_group = TRUE) |> print(n=150)

data[too_few_motifs, c("motif_count.x", "motif_count.y", "song_count")] |> print(n=150)
