...
- Main change: Substitute 'motif' for 'song' given it is more accurate. -- THIS IS INCORRECT. THIS DATA IS AT THE MOTIF LEVEL!
...
song_data_raw[[1]] <- read.csv(file.path(git_root, "data", "chamber-temp-and-humidity", "HSPi-Round-1-Heat-Trials.csv")) %>% mutate(round = 1) %>%
...
song_data_raw[[2]] <- read.csv(file.path(git_root, "data", "chamber-temp-and-humidity", "HSPi-Repeatability-Song-Count.csv")) %>%
    mutate(round = 2) %>%
    ungroup()

song_data_raw[[3]] <-read.csv(file.path(git_root, "data", "chamber-temp-and-humidity", "HSPi-Round-2-Heat-Trials.csv")) %>%
    mutate(round = 3) %>%
...
song_data_full <- full_join(song_data_raw[[1]], song_data_raw[[2]]) %>%
    ...
    rename(song_count = motif_count,
           song_rate = motif_rate) %>%
    ...
    mutate(n_obs_completed = !(is.na(song_count)),
           ...
           song_count = ifelse(is.na(song_count), 0, song_count)) %>%
#    mutate(song_count = song_count*1.0) %>% ## convert to a double so it's not treated as an integer
    ...
    ungroup() %>%
    mutate(song_count_plus_1 = as.integer((song_count + 1))) %>%
    ...
    mutate(temp = if_else(is.na(temp_mean),
                          temp_target,
                          temp_mean)) %>%
    ## Add column with a male's total song_count for a given round
    group_by(male, round) %>%
  ...
  mutate() #song_count = as_integer(song_count),
...
save_obj <- c("song_data_full", "song_data", "song_data_40C", "song_stats", "song_stats_40C", "bird_bill_data")
