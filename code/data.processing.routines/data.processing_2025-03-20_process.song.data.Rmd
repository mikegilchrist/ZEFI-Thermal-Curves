...
song_summary <- song_data |>
  group_by(male, date) |>
    summarize(song_count = if_else(sum(motif) > 0, max(song_index), 0),
              motif_count_song_only = sum(motif),
            ...
            post_song_motif_freq = post_song_motif_occurances/(motif_count_song_only)
            ) %>% 
    left_join(song_data_misc) %>% 
    mutate(motif_misc = if_else(is.na(motif_misc), 0, motif_misc)) %>%
    mutate(motif_count = motif_count_song_only +  motif_misc, .before = song_count) %>%
    identity()
