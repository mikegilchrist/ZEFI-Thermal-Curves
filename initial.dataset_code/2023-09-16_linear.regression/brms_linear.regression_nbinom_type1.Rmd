...
- 2023-09-16: Focus on using linear regression
  - Error assumptions
    - Log normal
    - nb_type1
...
- Fit simple linear regression to potentially only the third set of data.
...
> Note using disp instead of theta because theta is reserved within `brms` (?) 
...
ggplot2::theme_update(text = element_text(size = 10)) #base_size = 10)
...
options(cmdstanr_write_stan_file_dir = file.path("output", "cmdstanr"))
...
data_lst <- list()
for(i in 1:3) {
  data_lst[[i]] <- data_ind %>% filter(round == i)
}
...
data_log10_lst <- list()
for(i in 1:3) {
  data_log10_lst[[i]] <- data_lst[[i]] %>% mutate(motif_count = motif_count + 1)
}
...
data_summary_stats <- data_lst[[round]] %>%
  group_by(male) %>%
  #filter(temp <=40) %>% 
  summarize(mean = mean(motif_count),
            sd = sd(motif_count),
            var = sd^2,
            cv = sd(motif_count)/mean(motif_count),
            var_to_mean = var/mean,
            var_to_mean_sqr = var/(mean^2),
            phi = mean^2/(var - mean) # phi is the overdispersion parameter in the type 2 NB
            )

phi_median <- median(data_summary_stats$phi)
phi_mean <- mean(data_summary_stats$phi)
phi_sd <- sd(data_summary_stats$phi)


hist <- ggplot(data_summary_stats, aes(phi)) +
  geom_histogram(bins = 10)
plot_grid(plotlist = list(hist, hist + scale_x_log10()))

plot_pairs <- pairs(data_summary_stats %>% select(-c(male, cv, sd, var_to_mean_sqr)))

hist <- ggplot(data_summary_stats, aes(var)) +
...
gg_data <- ggplot(data_lst[[round]])
gg_points <- gg_data + geom_point(aes(temp, motif_count))
gg_grid <- gg_points + facet_wrap(vars(male))


gg_grid_log <- gg_grid + scale_y_log10()
plot_grid(plotlist = list(gg_grid, gg_grid_log))
...
data <- data_lst[[round]] %>%
  left_join(y=data_summary_stats, by = "male") %>%
  filter(var < 10000)

if(!exists("fit_lst")) fit_lst <- list()

formula_linear <-  bf(motif_count ~ 1 + temp +  (1 + temp||male))
                     
fit_lst[["normal"]] <- brm(formula = formula_linear, 
           data = data,
           control = list(adapt_delta = 0.85),                          
           iter = 20000,
           warmup = iter*0.75,
           thin  = 1,
           prior = prior(uniform(1, 500), class = "Intercept", lb = 1, ub = 500),
           backend = "cmdstanr",
           file = "linear_normal" # files to be saved in output/cmdstanr
           )

fit <- update(fit_lst[["normal"]],
              #              thin = 10,
              iter = 100000,
              warmup = iter*0.5,
...
fit <- fit_lst[["nbinom_1"]] <- brm(formula = formula_linear,
           data = data,
           ...
           thin  = 1,
           prior = prior(uniform(1, 500), class = "Intercept", lb = 1, ub = 500) +
             prior(gamma(0.1, 0.1), class = "disp", lb = 0),
           #backend = "cmdstanr",
           #file = "linear_nbinom_1"
           )

fit <- update(fit_lst[["nbinom_1"]],
              #              thin = 10,
              iter = 50000,
              thin = 10,
              warmup = iter*0.75,
...
models <- c("linear", "linear_threshold")
